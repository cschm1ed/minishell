...
/*   Updated: 2023/05/12 15:32:34 by lspohle          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_child_process(t_data *pipex, t_list *parsed, t_info *info)
{
	t_parsed  *content;
	...
	if (ft_strncmp(content->cmd, pipex->argv[pipex->argc - 2], sizeof(content->cmd)) == 0)
	{
		close(pipex->pipe_fd[1]);
	...
	if (execve(pipex->cmd_path, content->args, pipex->envp) <= -1) 
